...
2.Use an AlphaZero-style approach to optimize parameters. The key difference is that we allow growth of the embedding matrix: existing parameters are kept fixed, and reinforcement learning is used to update the parameters of the expanded embedding matrix.
...
2、用AlphaZero的方法优化参数，区别在于我们允许嵌入矩阵的增生，方法是先保持原有参数，再用强化学习更新增生后的嵌入矩阵的参数。
